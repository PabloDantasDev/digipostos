...
## Para rodar a aplicação usando docker rode os seguintes comandos
...
```bash
docker exec -it prefeiturapostos-app-1 bash
```
...
6. Popular a tabela de combustiveis
```bash
php artisan db:seed
```

7. Instalar as depedências do template web (necessário node 18)
```bash
npm install
```

8. Rodar a parte web do projeto
```bash
npm run dev
```

9. Para acessar aplicação acesse:
```bash
http://localhost:8989
```
